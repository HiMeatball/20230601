...
class Bullet{
    constructor(args){ //預設值,基本資料(物件的顏色,移動的速度,大小,初始顯示位置.....)
        ...
        this.v = args.v || createVector(mouseX-width/2,mouseY-height/2).limit(10) //根據 args.v 的值，或根據鼠標位置創建一個向量並限制其長度在 10 的範圍內，然後將該向量賦值給 this.v 變數。
        this.color = args.color || "#001d3d"
    }
    draw(){ //繪出物件程式碼
        push()
